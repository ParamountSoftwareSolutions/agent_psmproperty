                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h4>:Leads Table</h4>
                            <a href="{{url('/'.(new App\Helpers\Helpers)->user_login_route()['panel'] . '/webhook/lead_assign_to_mangers/' . $form_id . '/' . $token)}}" class="btn btn-primary" style="margin-left: auto; display: block;">Assign To Managers</a>
                            <a href="{{url('/'.(new App\Helpers\Helpers)->user_login_route()['panel'] . '/webhook/lead_assign_to_sale_person/' . $form_id . '/' . $token)}}" class="btn btn-primary" style="margin-left: auto; display: block;">Assign To Sale Person</a>
                        </div>
                        <div class="card-body">
                            <ul id="list"></ul>
                                    ...
                                    <tbody>
                                        <?php if (!empty($data->data)) { ?>
                                            <?php foreach ($data->data as $key => $val) { ?>
                                                <?php
                                                $fields = array_reduce($val->field_data, static function ($fields, $entry) {
