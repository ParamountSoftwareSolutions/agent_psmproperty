...
class CreateBuildingSaleHistoriesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('building_sale_histories', function (Blueprint $table) {
            $table->id();
            $table->string('key');
            $table->foreignId('building_sale_id')->unsigned()->nullable()->constrained('building_sales')->nullOnDelete();
            /*$table->unsignedBigInteger('building_sale_id');
            $table->foreign('building_sale_id')->references('id')->on('building_sales')->onUpdate('cascade')->nullOnDelete();*/
            $table->string('data');
