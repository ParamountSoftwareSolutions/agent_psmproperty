...
class FloorDetailController extends Controller
{
    ...
    public function store(Request $request, $building_id, $floor_id)
    {
        ...
        $floor_detail->save();
        if ($floor_detail) {
            return redirect()->route('property.floor_detail.index', ['building_id' => $building_id, 'floor_id' => $floor_id])->with($this->message('Data Create SuccessFully', 'success'));
        } else {
    ...
    public function update(Request $request, $building_id, $floor_id, $id)
    {
        ...
        $floor_detail->save();
        if ($floor_detail) {
            return redirect()->route('property.floor_detail.index', ['building_id' => $building_id, 'floor_id' => $floor_id])->with($this->message('Data Update SuccessFully', 'success'));
        } else {
    ...
    public function destroy($building_id, $floor_id, $id)
    {
        ...
        $floor_detail->delete();
        if ($floor_detail) {
            return redirect()->route('property.floor_detail.index', ['building_id' => $building_id, 'floor_id' => $floor_id])->with($this->message('Data Delete SuccessFully', 'success'));
        } else {
