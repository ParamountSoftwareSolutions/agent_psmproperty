...
class SettingController extends Controller
{
    public function push_notification()
    {
        ...
        if (BuildingSetting::where(['user_id' => Auth::id(), 'user_type' => Auth::user()->roles[0]->name, 'key' => 'sale_lost'])->first() == false) {
            BuildingSetting::insert([
                'user_id' => Auth::id(),
                'user_type' => Auth::user()->roles[0]->name,
                'key' => 'sale_lost',
    ...
    public function update_server_key(Request $request)
    {
        ...
        if ($server_key) {
            return redirect()->route('property.setting.push_notification')->with(['alert' => 'success', 'message' => 'Server Key Update Successfully']);
        } else {
    ...
    public function update_push_notification(Request $request)
    {
        ...
        BuildingSetting::updateOrInsert(['user_id' => Auth::id(), 'user_type' => Auth::user()->roles[0]->name, 'key' => 'sale_lost'], [
            'value' => json_encode([
                'status' => $request['sale_lost_status'] == 'on' ? 1 : 0,
                'message' => $request['sale_lost_message'],
            ]),
        ]);
        ...
        return redirect()->route('property.setting.push_notification')->with(['alert' => 'success', 'message' => 'Notification Update Successfully']);
    }
}
